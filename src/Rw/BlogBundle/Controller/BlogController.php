...
class BlogController extends Controller
{
	...
	public function deleteCommentAction($id)
	{
		...
		if($comment === null)
		{
			throw $this->createNotFoundException('Commentaire[id='.$id.'] inexistant.');
		}
		$billet = $comment->getBillet();
		// On crée un formulaire vide, qui ne contiendra que le champ CSRF
		// Cela permet de protéger la suppression d'article contre cette faille
		...
		if ($request->getMethod() == 'POST') {
			...
			if ($form->isValid()) { // Ici, isValid ne vérifie donc que le CSRF
				...
				$em->remove($comment);
				$em->flush(); // Exécute un DELETE sur $comment
				// On définit un message flash
				$this->get('session')->getFlashBag()->add('info', 'Le commentaire a bien été supprimé !');
		        // Puis on redirige vers l'accueil
				return $this->redirect($this->generateUrl('rwblog_view', array('id' => $billet->getId())));
			}
		}
		// Si la requète est en GET, on affiche une page de confirmation avant de supprimer
		return $this->render('RwBlogBundle:Blog:deletecomment.html.twig', array(
		'comment' => $comment,
		'form'    => $form->createView()
		));
	}
